...
for image_path in Path("watchlist").glob("*.png"):
...
threshold = 0.5
if(best_face_distance < threshold): 
    similarity = (1 - best_face_distance)* 100
    sim = round(similarity,2)   
    print("The similarity between the suspected morphed image and the suspected adversary is ", sim,"%")
    pil_image = Image.fromarray(best_face_image)
    pil_image.show()
else:
    print("None of the faces from the watchlist match the given image")
